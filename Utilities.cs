...
namespace MGBW_ENG_Commands
{
    public static class Utilities
    {
                                                                       ...
                                                                       string IconFolder = "")
        {
            ...
            if (!(testDef == null))
            {
                MessageBox.Show("Error when loading the add-in \"Inventor_MGBW_Commands\". A command already exists with the same internal name. Each add-in must have a unique internal name. Change the internal name in the call to CreateButtonDefinition.", "CSharp Inventor Add-In Template");
                return null;
            }
            ...
            {
                MessageBox.Show("line 131. i dont know what would break it /n /n" + ex.ToString());
